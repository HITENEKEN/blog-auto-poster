...
| 플랫폼 지연 초기화   | 재기동 직후 `/health`의 `services.platforms`가 `{}`였다(2026-09-15 `pm2 resurrect` 직후). 설정은 그대로였고, 대시보드를 쓴 뒤 다시 조회하니 `naver: true`가 나왔다(2026-09-21). 어댑터는 `platformRegistry.getAdapter`가 처음 불릴 때 만들어지고, `/health`는 이미 만들어진 인스턴스만 `validateAll()`한다(`src/web/server/index.ts:205`, `src/platforms/registry.ts:36-49`) | `/health`가 **설정된 플랫폼을 항상 보고**하도록 고친다(PR4, §4-1). 고치기 전까지도 로봇이 막히지 않도록 `PREFLIGHT`에 깨우기 절차를 둔다(§5-1). 스킬 S0의 `platforms.naver == true` 검사도 재기동 직후에는 같은 이유로 실패한다 |
...
CREATE TABLE IF NOT EXISTS robot_runs (
  ...
  code_version TEXT,                      -- src/robot/codeVersion.ts: env BLOG_POSTER_CODE_VERSION → git rev-parse HEAD
                                          --   (+'-dirty' when git status --porcelain is non-empty) → 'unknown'. 빈 문자열 금지
...
템플릿 6종에는 **정적 1인칭 체험 문구가 없다**(회귀 테스트 `tests/unit/template-copy.test.ts`) — 예전 기본 도입부가 만들어 내던 허위 경험담을 발행 전에 지우는 대신 템플릿에서 제거했다.
...
| 메서드·경로                         | 요청                                                             | 응답                                                                                                 | 비고                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /health`                       | —                                                                | 기존 + `services.naverSession: {expiresAt, daysLeft}`                                                | 프로필 `Cookies` DB를 읽기 전용으로 열어 `NID_SES`·`NID_AUT` 만료를 읽는다(스킬 S0과 같은 쿼리). **`services.platforms`는 설정에 있는 플랫폼 전체를 보고한다**: `validateAll()` 전에 `Object.keys(config.platforms)`를 돌며 `getAdapter(name)`로 인스턴스를 만든다(`/api/blogs`가 이미 쓰는 방식). 재기동 직후 `{}`가 나오던 문제(§0)를 없애고 응답 모양은 그대로 둔다 |
...
| 단계            | 호출·동작                                                                                                                                                                                                                                               | 산출                                                      | 실패                                  |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------- |
| `PREFLIGHT`     | `GET /health`(naver true·scheduler stopped·`naverSession.daysLeft ≥ 14`), 로그인, `paused` 확인. `platforms`가 비어 있거나 `naver` 키가 없으면 **깨우기**: `GET /api/blogs` 1회 호출 후 `/health` 재조회(최대 2회). 그래도 `naver`가 true가 아니면 중단 | —                                                         | `aborted-health`/`-login`/`-session`  |
...
| `RECORD`         | `run.json`(확정 status·`finished_at` 포함), `report.md`, `publish-log.jsonl`(runId가 이미 있으면 추가 안 함), `consecutive_pass` 갱신, 인벤토리 `used_count` 반영 — `PATCH /api/ads/inventory/:id {markUsed:true}`(`DashboardClient.markAdUsed`), 실행·광고별 `robot_state` 키 `ads-used:<runId>:<adId>`로 재실행·크래시 재개에도 이중 집계 금지, 실패는 실제 경고 `ads-mark-used-failed:<id>`로 남긴다, 알림 | runId                                         | —                                                                               |

**금지 문구 표의 단일 출처**: 규칙 표는 `src/content/ForbiddenText.ts` 하나이며 광고 게이트와 `checkEditedDraft`(EDIT)가 함께 본다. 광고 문구 스코프 `AD_TEXT_CODES`는 `UNSUPPORTED_PRICE`를 제외한다 — 판매자가 쓴 상품명에 "100% 정품 보장"·"최저가 보장"이 정당하게 들어가기 때문이다. 초안 본문 스코프 `DRAFT_TEXT_CODES`는 그대로 포함한다.
...
| `isDemandOk(series)`               | `classifyTrend(series) !== 'falling'` 이면서 최근 4주 마지막 값 ≥ 최근 4주 최댓값 × 0.8. **구현은 비교 전에 아직 끝나지 않은 마지막 주를 버린다**(`policies.dropPartialWeek`) — 검색어트렌드 `timeUnit=week`는 진행 중인 주를 부분 집계로 돌려준다(실측 2026-09-16: 2026-09-14 구간 21.34 vs 완결 주 81–87). `filterCandidates`·`rankTopics`도 같은 완결 주 시계열을 쓴다 | §5 조건 1      |
...
| 항목                 | OK 조건                                                                                              |
| -------------------- | ---------------------------------------------------------------------------------------------------- |
| web 서버             | `/health` 200, naver true(깨우기 후에도 false면 FAIL), scheduler stopped                             |
...
| 4   | 운영 기반: `ecosystem.config.cjs`, `.env`, `/health` 세션 + **설정 기준 플랫폼 보고**(§0·§4-1), `NaverRss.ts` 분리, README env 문구 수정, 보안 기본값 교체 | `pm2 startup` 후 재부팅해도 web 자동 기동. 재기동 직후 `/health`에 `naver: true`가 바로 나온다(유닛 테스트: 인스턴스 0개 상태에서 설정된 플랫폼이 모두 보고됨) |
