...
/**
 * Drives LLM-based, 기준형(객관 구매 가이드) Korean blog copy generation.
 *
...
export class ContentGenerator {
  ...
  buildPrompt(
    ...
    const system = [
      ...
      '경험담이 아니라 **기준형(객관 구매 가이드)** 화법으로 작성하세요: 선택 기준, 비교 포인트, 확인 항목을 제시합니다.',
      '1인칭 체험 화법은 금지입니다(제가/저는/써봤/입어봤/내돈내산/직접 써·사용·입어). 주어를 생략하거나 "구매자는", "이 제품은"처럼 쓰세요.',
    ...
    const user = [
      ...
      bench,
      '',
      '위 정보를 바탕으로 다음 6개 필드를 한국어 기준형(객관 구매 가이드) 문체로 작성해 주세요.',
      '문자열 필드는 문단 단위 텍스트, 배열 필드는 항목 문자열 배열입니다.',
      '출력 JSON 스키마:',
      '{',
      '  "experienceIntro": "독자에게 던지는 공감형 도입부. 어떤 상황에서 이 상품을 찾게 되는지(구매 목적·사용 상황)를 기준형으로. 4~6문장.",',
      '  "realUsageStory": "제품이 어떤 방식으로 동작하고 무엇을 확인해야 하는지 설명하는 본문. 사용 환경, 소요 시간, 근거 있는 수치, 장점과 한계를 포함해 8~12문장. 경험담이 아니라 기준 설명으로 씁니다.",',
  ...
  private benchmarkSection(topPosts: TopPost[]): string {
    return topPosts.length
      ? topPosts
          ...
          .join('\n')
      : '(벤치마크 데이터 없음 — 공개된 제품 정보와 구매 기준을 바탕으로 작성하세요)';
  }
  ...
  /**
   * buildPrompt와 동일한 시스템 규칙(기준형 문체, 벤치마크 표절 금지)에
  ...
  buildTemplateDataPrompt(req: TemplateDataRequest): { system: string; user: string } {
    ...
    const user = [
      ...
      bench,
      '',
      '위 정보를 바탕으로 아래 필드를 한국어 기준형(객관 구매 가이드) 문체로 작성해 주세요.',
