...
import { Globe, RefreshCw, CheckCircle, XCircle, ExternalLink, Plus, Settings } from 'lucide-react';
...
export default function Blogs() {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      toast({ title: '오류', description: '블로그 목록을 불러오는데 실패했습니다.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSyncCategories = async (blogName: string) => {
    setSyncing(blogName);
    try {
      const response = await api.post(`/api/blogs/${blogName}/sync-categories`);
      toast({ title: '성공', description: '카테고리가 동기화되었습니다.' });
      fetchBlogs();
    } catch (error) {
      toast({ title: '오류', description: '카테고리 동기화에 실패했습니다.', variant: 'destructive' });
    } finally {
      setSyncing(null);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">연동된 블로그 플랫폼을 관리하고 발행 현황을 확인하세요</p>
        </div>
      </div>
            ...
            <Card key={blog.name} className="overflow-hidden">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <div className="flex items-center gap-3">
                  <div className={clsx('h-10 w-10 rounded-lg flex items-center justify-center', blog.connected ? 'bg-green-100' : 'bg-red-100')}>
                    <Globe className={clsx('h-5 w-5', blog.connected ? 'text-green-600' : 'text-red-600')} />
                  </div>
                  <div>
                ...
                {blog.lastPost && (
                  <div className="p-3 rounded-lg bg-muted/50">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {new Date(blog.lastPost.publishedAt).toLocaleDateString()} • {blog.lastPost.status}
                    </p>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {blog.categories.length > 5 && (
                        <Badge variant="outline" className="text-xs">+{blog.categories.length - 5}개 더</Badge>
                      )}
                    </div>
                  </div>
                )}
