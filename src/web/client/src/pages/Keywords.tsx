...
function TrendBadge({ trend }: { trend: string }) {
  if (trend === 'rising')
    return <Badge className="bg-green-100 text-green-700"><TrendingUp className="h-3 w-3 mr-1" />상승</Badge>;
  if (trend === 'falling')
    return <Badge className="bg-red-100 text-red-700"><TrendingDown className="h-3 w-3 mr-1" />하락</Badge>;
  return <Badge variant="outline"><Minus className="h-3 w-3 mr-1" />보합</Badge>;
}
...
export default function Keywords() {
  ...
  const handleSearch = async () => {
    ...
    setError('');
    try {
      const response = await api.get(`/api/keywords/trending?q=${encodeURIComponent(query)}&limit=${limit}`);
  ...
  const sortedKeywords = result?.trending?.sort(
    ...
    <div className="space-y-6">
      ...
      {loading && (
        <Card><CardContent className="p-12 flex items-center justify-center">
          ...
          <span className="ml-3 text-muted-foreground">네이버 API Hub에서 데이터를 가져오는 중...</span>
        </CardContent></Card>
      )}
                ...
                <tbody>
                  {sortedKeywords.map((kw, i) => (
                    <tr key={kw.keyword + i} className="border-b hover:bg-muted/50 transition-colors">
                      ...
                      <td className="py-3 pr-4">{kw.volume > 0 ? `~${kw.volume.toLocaleString()}` : '-'}</td>
                      <td className="py-3 pr-4"><CompetitionBar value={kw.competition} /></td>
                      <td className="py-3 pr-4"><TrendBadge trend={kw.trend} /></td>
